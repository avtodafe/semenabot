...
from bot import send_daily_report, send_text
...
for kw in SEARCH_TERMS:
    ...
    if results:
        ...
        kw_results.append(f"  '{kw}' → 0 (ошибка или пусто)")
...
# Diagnostic status message (always sent)
status_lines = [
    "🔍 Roseltorg диагностика:",
    f"Получено: {len(all_raw)}, после фильтра: {len(filtered)}, новых: {saved}, к отправке: {len(tenders)}",
] + kw_results[:10]
try:
    send_text("\n".join(status_lines))
